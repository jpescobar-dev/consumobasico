    ...
    <div id="content" class="main-content">
        <div class="layout-px-spacing">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <!-- Contenido derecho -->
                    <a href="{{ route('catalogos.create') }}" class="btn btn-outline-primary btn-sm" title="Nuevo Catálogo">
        ...
        <tbody>
            @foreach ($catalogos as $catalogo)
                <tr>
                    ...
                    <td>{{ Str::limit($catalogo->descripcion, 80) }}</td>                
                    <td>{{ $catalogo->estado }}</td>
                    <td>{{ $catalogo->item ?? 'No asignado' }}</td>
                    <td class="text-center">
                        <div class="d-inline-flex align-items-center" style="gap: 2px;">
                            ...
                            <form action="{{ route('catalogos.destroy', $catalogo) }}" method="POST" onsubmit="return confirm('¿Está seguro de eliminar este catálogo?');">
                                @csrf
                                @method('DELETE')
